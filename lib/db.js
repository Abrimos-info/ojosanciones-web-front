...
async function queryKeywords(filters) {
    ...
    Object.keys(filters).map(f => {
        ...
        if (filter.value  && f != "sort" && f != "sort-direction") {
...
async function queryStats() {
    ...
    const stats = {
        function_publica_count: result.aggregations.fuente.buckets[0].count.value,
        sat_count: result.aggregations.fuente.buckets[0].count.value,
        ofac_count: result.aggregations.fuente.buckets[0].count.value,
    }
    return stats;
}
