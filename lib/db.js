...
async function queryKeywords(keyword,country,type,page) {
    if (type == "buyer") {
        return queryBuyer(keyword,page);
    }
    let index = "";
    let size = pageSize;
    let from = size*page || 0;
    let highlight = {};
    let sort = {}
    index = "gob_"+type+"}";
...
module.exports = {
    connect, queryItem, queryKeywords, queryStats, pageSize
}