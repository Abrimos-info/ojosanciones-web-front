...
async function itemPage  (req,type,rel) {
    ...
    if (result) {
        ...
        html = pug.renderFile("templates/"+type+".pug",{item: result, pages: result.pages, ... pugLocals});
        status = 200;
    }
    else {
        console.log(new Date(), "FORCED404", type, "id",id)
        html = pug.renderFile("templates/"+type+".pug",{item: {}, ... pugLocals});
        status = 200;
...
async function buscadorPage(req,res) {
    ...
    const filters = {
        ...
        "autoridad":{
            ...
            options: [
                { value: "", label: "Todas" },
                { value: "funcion-publica", label: "Secretaría de la Función Pública (SFP, MX)"},
                { value: "sat", label: "Secretaría de Administración Tributaria (SAT, MX)"},
        ...
        "sort":{
            type: "hidden",
            options: [
                {value: "date", label: "fecha última sanción"}
            ],
            defaultValue: "date"
        },
        "sort-direction":{
