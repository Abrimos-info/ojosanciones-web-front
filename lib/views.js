...
async function itemPage  (req,type,rel) {
    ...
    if (!last_update) { let stats = await db.queryStats(); last_update = stats.last_update; }
    console.log(new Date(), "itemPage last_update", last_update);
    result = await db.queryItem(type, id, country, rel);
    // console.log(result);
    if (result) {
        console.log(new Date(), type, "id",id)
        html = pug.renderFile("templates/"+type+".pug",{item: result, filters: filters, pages: result.pages, last_update: last_update, ... pugLocals});
        status = 200;
    }
    else {
...
const views = {
    ...
    empresaPage: async function   (req,res) {
        return await itemPage(req,"sancion")
    },
    
    
    buscadorPage: async function (req,res) {
        let url = new URL(req.url, "http://localhost");
    
        // Check if filters is properly initialized
        if (!filters || typeof filters !== 'object') {
            console.error("Filters not properly initialized");
            return {
                html: pug.renderFile("templates/error.pug", { 
                    error: "Error: Filters not properly initialized",
                    ...pugLocals 
                }),
                status: 500
            };
        ...
        let locals = {
            ...
            currentPage: parseInt(filters.page?.value || 1)
        }
    ...
    //countryPage:  async function (req,res) {
    //     let url = new URL(req.url, "http://localhost");
    //     let country = url.pathname.split("/")[1];
...
const requestListener = async function (req, res) {
    ...
    let route = null;
    localRoutes = await routes;
    // console.log(localRoutes);

    try {
        let routeLevel1 = url.pathname.split("/")[1];
        let routeLevel2 = null;
        // console.log("requestListener routeLevel1",routeLevel1);
        ...
        if (!localRoutes.hasOwnProperty(routeLevel1)) {
            routeLevel1="default";
            // console.log("requestListener default",routeLevel1);
        }

        if (routeLevel1=="static") {
            // console.log("es estatic",routeLevel1)
            return sendStatic(req,res)
        }

        if (typeof localRoutes[routeLevel1].viewFunction == "function") {
            // console.log("requestListener routeLevel1",localRoutes[routeLevel1])
            responseData = await localRoutes[routeLevel1].viewFunction(req, res);    
        }
        else if (localRoutes[routeLevel1]) {
            
            routeLevel2 = url.pathname.split("/")[2];
            let localSecondLevelRoutes = await localRoutes[routeLevel1];
            localSecondLevelRoutes.viewFunction = await localSecondLevelRoutes.viewFunction;
            // let localRouteFunction = await localSecondLevelRoutes.viewFunction
            if (typeof localSecondLevelRoutes.viewFunction[routeLevel2].viewFunction == "function") {
                responseData = await localSecondLevelRoutes.viewFunction[routeLevel2].viewFunction(req, res);
            }
        } 
        else {
...
async function loadRoutes(type) {
    ...
    const typesFile = "./routes/"+type+".json";
    try {

        var json = JSON.parse(fs.readFileSync(typesFile, 'utf8'));

        // console.log(json)
        Object.keys(json).forEach(item => {
            if (item == "meta") return;
            routes[item] = json[item];
            switch (routes[item].viewType) {
                case "static":
                    routes[item].viewFunction = views.staticPage(json[item].viewName)
                    break;
                case "country":
                    routes[item].viewFunction = countryRoutes
                    break;
                case "function":
                    routes[item].viewFunction = views[routes[item].viewName]
                    break;
                }
        })
    }
    catch (e) {
        console.error("loadRoutes",e);
    }
    return routes;
    
}
const countryRoutes = loadRoutes("countryRoutes")
...
module.exports = { routes, requestListener, clearTemplateCache }